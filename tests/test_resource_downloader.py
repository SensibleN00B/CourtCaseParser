...
def test_parse_total_from_content_range():
    assert resource_downloader._parse_total_from_content_range('bytes 0-1023/2048') == 2048
...
class FakeResponse:
    def __init__(self, status_code: int = 200, headers: Optional[dict] = None, content: bytes = b''):
...
class FakeSession:
    ...
    def get(self, url, stream=True, timeout=None, headers=None):
        return FakeResponse(200, headers={'content-length': str(len(self._content))}, content=self._content)
...
def test_download_file_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(resource_downloader, 'create_session', lambda: FakeSession(content))

    path = resource_downloader.download_file('https://example.com/file.bin', str(out_dir))
...
def test_download_all_files_aggregates_results(tmp_path, monkeypatch):
    ...
    out_dir = tmp_path / 'dl'
    results = resource_downloader.download_all_files(resources, str(out_dir), max_workers=2)
